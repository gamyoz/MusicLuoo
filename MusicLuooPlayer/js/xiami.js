$(window).load(function () {
    $("#tableList").css({ height: (window.screen.availHeight - 250) + "px", 'overflow-y': 'auto' });
...
$(function () {
    ...
    $(".start em").click(function () {
        /*开始放歌*/
        var sid = parseInt($(this).attr("sonIndex"));
        songIndex = sid;
        luoo_player.play(sid);
    });
    /*双击播放*/
    $(".songList").dblclick(function () {
        var sid = $(this).find(".start em").attr("sonIndex");
        luoo_player.play(sid);
    });
    /*底部进度条控制*/
    $(".dian").draggable({
    ...
    $(".mode").click(function () {
        if ($(this).hasClass("mode-loop")) {
            $(this).removeClass("mode-loop").addClass("mode-random");
        } else if ($(this).hasClass("mode-random")) {
            $(this).removeClass("mode-random").addClass("mode-single");
        } else {
            $(this).removeClass("mode-single").addClass("mode-loop");
        }
    });
    /*切歌*/
...
var luoo_player = {
    pageSize:100,
    init: function () {
        var count = parseInt(_songCount / luoo_player.pageSize);
        if (_songCount % luoo_player.pageSize > 0)
            count += 1;
        for (var i = 2; i < count; i++) {
            luoo_player.initByPager(i);
        }
    },
    initByPager: function(pageIndex) {
        ajax("/ajax/ajax.ashx?action=Init", "Get", { pageindex: pageIndex, pageSize: luoo_player.pageSize }, function (data) {
            $.each(data, function (i, j) {
                var html = "";
                html += '<tr class="songList" index="'+j["SongIndex"]+'">';
                html += '<td style="text-align: center">';
                html += '<input class="checkIn" type="checkbox" select="0" /></td>';
                html += '<td style="text-align: center">';
                html += '<div class="start">';
                html += '<em sonn="'+j["DownloadUrl"]+'" sonindex="'+j["SongIndex"]+'" volpic="'+j["VolPicUrl"].split('!')[0]+'">'+j["SongIndex"]+'</em>';
                html += '</div>';
                html += '</td>';
                html += '<td>';
                html += '<span id="songName'+j["SongIndex"]+'">'+j["SongName"]+'</span>';
                html += '</td>';
                html += '<td>';
                html += '<span id="songAuthor'+j["SongIndex"]+'">'+j["Author"]+'</span>';
                html += '</td>';
                html += '<td>';
                html += j["AlbumName"];
                html += '</td>';
    ...
    play: function (index) {
        ...
        luoo_player.play_status(index);
    },
    play_status: function (sid, isPause) {//显示播放状态
        if (isPause) {//暂停

        } else {//播放
            var $this = $(".start em[sonIndex=" + sid + "]");
            if ($this.length == 0) {
                luoo_player.next();
                return;
            }
            //$('.songUL').animate({
            //    scrollTop: $this.offset().top - $('.songUL').offset().top + $('.songUL').scrollTop()
            //},100);​
            //$("#tableList").css({ height: (window.screen.availHeight - 250) + "px", 'overflow-y': 'auto' });
            //$('#tableList').slimScroll({
            //    start: $(".songList[index=" + sid + "]"),
            //    height: 'auto'
            //});
            $("#tableList").mCustomScrollbar("scrollTo", ".songList[index=" + sid + "]");

            $("#audio").attr("src", $this.attr("sonN"));
            ...
            /*底部显示歌曲信息*/
            var songName = $("#songName" + sid).text();
            var singerName = $("#songAuthor" + sid).text();
            $(".songName").html("[" + (sid + 1) + "] " + songName);
            $(".songPlayer").html(singerName);
            /*换右侧图片*/
            var pic = $this.attr("volpic");
    ...
    random: function () {//获取随机数
        var max = parseInt($("em:last").attr("sonIndex"));
    ...
    next: function () {
        var type = 0;//循环
        if ($(".mode").hasClass("mode-random")) type = 1;
        else if ($(".mode").hasClass("mode-single")) type = 2;

        var sid = 0;
        switch (type) {
            case 1:
                sid = luoo_player.random();
                break;
            case 2:
                sid = songIndex;
                break;
...
function loadBG() {
    ...
    $("#canvas1").attr("src", "/files/picture/5a4666a4db0b1.jpg");
...
function audioPlay(ev) {
    ...
    $(".start em[sonIndex=" + songIndex + "]").css({
        "background": 'url("/css/images/T1X4JEFq8qXXXaYEA_-11-12.gif") no-repeat',
        "color": "transparent"
    });
}
function audioPause(ev) {
    ...
    $(".start em[sonIndex=" + songIndex + "]").css({
         "background": 'url("/css/images/pause.png") no-repeat',
         "color": "transparent"
     });
}
